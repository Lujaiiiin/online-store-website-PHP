...
<?php
            include "config.php";
            $i=1;
            	$myquery = mysqli_query($connection,"SELECT * FROM `products` WHERE ID=".$_GET['id']." ORDER BY id DESC ");
...
?>
<a style="color: black;" href="product.php?id=<?= $row['id']?>">
  <div style="display: inline-block;margin: 10px;" class="m-10">
  
   <div style="display: inline-block;" class="bg-img"> <img src= "<?= $row['img_url']?>" id="myImage1"  height="320" width="270" > </div>
    
    <div style="display: inline-block;" class="content">
     <h4><?= $row['name']  ?></h4>
	 <h6>SAR <?= $row['price']?></h6>
      <p> <?= $row['description']?></p> <button>Add to cart</button> </div>
